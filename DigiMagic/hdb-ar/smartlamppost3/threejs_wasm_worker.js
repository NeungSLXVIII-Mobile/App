...
loadTextures(urls, function (error, textures) {
    ...
    var p3_icon_text_material = new THREE.MeshBasicMaterial({ map: new THREE.TextureLoader().load("assets/sprites/p3_icon_text.png"), opacity: 1, depthTest: false, depthWrite: false, transparent: true, side: THREE.DoubleSide });
    p3_icon_text_move = new THREE.Mesh(new THREE.PlaneGeometry(150, 150), p3_icon_text_material);
});
...
function start2(container, marker, video, input_width, input_height, canvas_draw, render_update, track_update) {
    ...
    threeGLTFLoader4.load("Data/models/smartlamppost4.gltf", function (gltf) {
        ...
        if (p3_icon_plane != undefined) {
            ...
            // Text.
            p3_icon_text_move.position.x = p3_icon_plane.position.x + 25;
            p3_icon_text_move.position.y = p3_icon_plane.position.y;
            p3_icon_text_move.position.z = p3_icon_plane.position.z;

            p3_icon_text_move.renderOrder = 1;

            model4.add(p3_icon_text_move);
        }
...
function choice3_worker() {
    ...
    if (!p3_show) {
        ...
        p3_icon_text_move.renderOrder = 9;
